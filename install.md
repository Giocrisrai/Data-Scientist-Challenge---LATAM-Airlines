# Data Scientist Challenge - LATAM Airlines guide installation
...
activate data_scientist_challenge___latam_airlines
```
...
To move beyond notebook prototyping, all reusable code should go into the `data_scientist_challenge___latam_airlines/` folder package. To use that package inside your project, install the project's module in editable mode, so you can edit files in the `data_scientist_challenge___latam_airlines` folder and use the modules inside your notebooks :
...
```python
from data_scientist_challenge___latam_airlines.utils.paths import data_dir
data_dir()
```
